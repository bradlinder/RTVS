...
class TranscriptStoryMixin:
    ...
    def add_speaker_label_at(self, seg_idx, split_time, name=None):
        """Adds a speaker label break: splits the segment at split_time and
        assigns it a name (prompting for one if not supplied).

        A split alone is invisible in the rendered transcript -- two
        adjacent segments with the same underlying speaker are drawn as one
        continuous paragraph -- so this always resolves a name before
        calling split_segment_at_time, instead of leaving that to callers.
        """
        if not self.transcript or seg_idx < 0 or seg_idx >= len(self.transcript.get("segments", [])):
            self.statusBar().showMessage("Click inside a transcript line first.")
            return False
    ...
    def split_segment_at_time(self, seg_idx, split_time, new_speaker_name=None):
        ...
        When ``new_speaker_name`` is supplied, the new right-hand segment is
        assigned that speaker in the same undoable operation.
        """
        ...
        if words:
            left_words = [w for w in words if w.get("start", target_seg["start"]) < split_time]
            right_words = [w for w in words if w.get("start", target_seg["start"]) >= split_time]

            if not left_words or not right_words:
                self.statusBar().showMessage("Cannot split at the very beginning or end of a segment.")
                return False
            ...
            # Without word-level timing there is no exact word timestamp, so
            # place the break at the nearest proportional text position
            # instead of always splitting the segment in half.
            seg_text = target_seg.get("text", "").split()
            if len(seg_text) < 2:
                self.statusBar().showMessage("Cannot split a segment with fewer than two words.")
                return False
            ...
            if duration > 0:
                ratio = max(0.0, min(1.0, (float(split_time) - seg_start) / duration))
            else:
                ratio = 0.5
        ...
        new_overrides = {}
        for idx, spk in self.segment_speaker_overrides.items():
            if idx <= seg_idx:
                new_overrides[idx] = spk
            else:
                new_overrides[idx + 1] = spk
        ...
        self.log_activity(f"[SPEAKER] Added speaker label break at {format_time(split_time)} (Split Segment #{seg_idx})")
        self.save_project()
    ...
    def remove_speaker_label_at_segment(self, seg_idx):
        """Remove a speaker label by reassigning all audio and text in this section to the previous speaker."""
        if not self.transcript or "segments" not in self.transcript:
            return False

        segments = self.transcript.get("segments", [])
        if seg_idx <= 0 or seg_idx >= len(segments):
            return False  # No previous speaker to merge into

        self.flush_pending_transcript_undo() if hasattr(self, "flush_pending_transcript_undo") else None
        before_state = self._capture_project_state() if hasattr(self, "_capture_project_state") else None

        # 1. Target speaker is the previous segment's effective speaker
        prev_seg_idx = seg_idx - 1
        target_name = self.get_effective_speaker_name(prev_seg_idx, segments[prev_seg_idx])
        target_raw = self.segment_speaker_overrides.get(prev_seg_idx) or self.speaker_for_segment(segments[prev_seg_idx])

        # 2. Removed speaker at current segment
        removed_name = self.get_effective_speaker_name(seg_idx, segments[seg_idx])

        # 3. Find all contiguous segments in this section that belong to removed_name
        section_indices = []
        ...
        # 4. Resolve override key for target_name
        override_key = None
        if target_raw and self.display_speaker(target_raw) == target_name:
            override_key = target_raw
        elif prev_seg_idx in self.segment_speaker_overrides:
            override_key = self.segment_speaker_overrides[prev_seg_idx]

        if not override_key:
            override_key = f"SEG_{prev_seg_idx}_SPEAKER"
            self.speaker_names[override_key] = target_name
            self.segment_speaker_overrides[prev_seg_idx] = override_key

        # 5. Reassign all segments in the section to the target speaker
        for idx in section_indices:
            self.segment_speaker_overrides[idx] = override_key

        # 6. Reassign underlying diarization segments in this time window
        if self.diarization and isinstance(self.diarization, dict):
            sec_start = segments[section_indices[0]].get("start", 0.0)
            sec_end = segments[section_indices[-1]].get("end", sec_start)
            diar_segs = self.diarization.get("segments", [])
            updated_diar = False
            for d_seg in diar_segs:
                d_start = d_seg.get("start", 0.0)
                d_end = d_seg.get("end", d_start)
                if max(sec_start, d_start) < min(sec_end, d_end):
                    d_seg["speaker"] = target_raw or override_key
                    updated_diar = True
            if updated_diar:
        ...
        # Log, save, and redraw
        self.log_activity(
            f"[SPEAKER] Removed speaker label '{removed_name}' at segment #{seg_idx}; "
            f"reassigned {len(section_indices)} segment(s) to '{target_name}'"
        )
        self.save_project()
        self.render_transcript()
        self.statusBar().showMessage(f"Reassigned '{removed_name}' to '{target_name}'.")
        return True
